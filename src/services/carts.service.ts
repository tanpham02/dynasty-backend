...
class CartService extends CRUDService<Carts> {
  ...
  async addOrUpdateCartItem(customerId: string, req: Request) {
    const cartRecord = await this.model.findOne({ customerId: customerId }).lean();
    ...
    if (cartRecord?.products && cartRequestBody) {
      const cartItemMatching = cartRecord.products.find(
        (productItemRecord: any) =>
          cartRequestBody?.product?.toString() === String(productItemRecord?.product)?.toString(),
      );
      ...
      return await this.model.findOneAndUpdate(
        { 'products._id': dataUpdate._id ?? cartItemMatching?._id },
        {
          $set: {
